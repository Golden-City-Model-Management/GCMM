...
const Men: NextPageWithLayout = () => {
  ...
  const { data, error, size, setSize } = useSWRInfinite(
    (index) =>
      `${process.env.NEXT_PUBLIC_SERVER_URL}/models?gender=male&limit=${PAGE_SIZE}&page=${index + 1
      }`,
    fetcher
  );
  ...
  return (
    <Box component='article' sx={error  && models.length === 0 ? { display: 'flex', justifyContent: 'center', 
      ...
      <InfiniteScroll
        ...
        endMessage={(!error ) ? 
        <Typography variant='h2' textAlign='center' component='p'>Models will be updated soon...</Typography> :
          models.length === 0 && 
